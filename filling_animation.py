...
def main():
    ...
    for j, label in enumerate(injections.keys()):
        ...
        while True:
            ...
            plt.text(0.01, 0.99, f"Injection pressure: {injection_pressure}", transform=plt.gca().transAxes, fontsize=12, verticalalignment='top')
            plt.text(0.01, 0.96, f"Filling: {int(len(filled_vessels) / vessel_count * 100)}%", transform=plt.gca().transAxes, fontsize=12, verticalalignment='top')
            ...
            new_filled_vessels = set([(11, injection_location)]) if injection_pressure else set()
            for vessel in filled_vessels:
                if vessel not in graph.edges: vessel = (vessel[1], vessel[0])
