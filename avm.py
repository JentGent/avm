...
def display(graph: nx.Graph):
    ...
    if xs:
        ...
        for node, coords in pos.items():
            if node not in NODE_POS.keys():
                pos[node] = [0.1 + 0.55 *
                             (coords[0] - mx) / Mx, 1.15 * (coords[1] - my) / My - 0.75]
    node_colors = {
    ...
    edge_widths = [(edge[2]["Δpressure"] * 0.0001 + 1)
                   if "Δpressure" in edge[2] else 1 for edge in graph.edges(data=True)]
    edge_colors = [abs(edge[2]["flow"]) if "flow" in edge[2]
                   else 1 for edge in graph.edges(data=True)]
    node_displays = nx.draw_networkx_nodes(graph, pos, node_color=[
                                           node_colors[node] for node in graph.nodes()], cmap=plt.cm.Reds)
    ...
    edge_labels = {(edge[0], edge[1]): str(
        round(abs(edge[2]["flow"]), 3)) for edge in graph.edges(data=True)}
    nx.draw_networkx_edge_labels(graph, pos, edge_labels)
    sm = plt.cm.ScalarMappable(cmap=plt.cm.cool, norm=plt.Normalize(
        vmin=min(edge_colors), vmax=max(edge_colors)))
    sm.set_array([])
    plt.colorbar(sm, label="Flow (mL/min)")
    sm = plt.cm.ScalarMappable(cmap=plt.cm.Reds, norm=plt.Normalize(
...
def simulate(intranidal_nodes: list, edges: list[list], pressures: dict[str, float], num_nidi: int, num_expected_edges: int):
    ...
    Args:
        ...
        edges (list[list]): List of lists of vessel data.
        pressures (dict[str, float]): A dictionary of known node : pressure values.
    ...
    for i in range(num_nidi):
        ...
        for j in range(num_intranidal_verts - 1):
            for k in range(2):
                ...
                while graph.has_edge(intranidal_nodes[j], intranidal_nodes[connect]) or connect == j:
                    connect = random.randint(0, num_intranidal_verts - 1)
                graph.add_edge(
        ...
        print([(graph.nodes[node], pressures[node])
              for node in pressures if node in graph.nodes])
        display(graph)
...
def main():
    results = simulate(INTRANIDAL_NODES, VESSELS, PRESSURES, 1, 93)
    for result in results:
        flow, pressure = result
        ...
        print(f"Minimum pressure: {np.min(np.abs(pressure / 1333.22))}")
        print(f"Maximum pressure: {np.max(np.abs(pressure / 1333.22))}")
        print(f"Average pressure: {np.average(np.abs(pressure / 1333.22))}")
